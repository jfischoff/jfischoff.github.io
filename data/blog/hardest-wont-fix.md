...
One might think `threadWaitRead` (and related functions) will block until a socket recieves new data for reading.
...
After registering for the notification `server` waits for the client to send something by calling `recv`.
...
Unfortunately this is not a bug. `threadWaitReadSTM` inherits the semantics of the IO multiplexing APIs that are used to implement the `EventManager` backends and this is how they are supposed to work.
...
On Linux `threadWaitReadSTM` is implemented by ultilizing `epoll_wait` to register interest in file descriptor ready events.
...
I discovered this tidbit of Linux trivia while trying to understanding this bug with `hasql-notifications` which I need to write `hasql-queue` my PostgreSQL queue library.

![issues](./hardest-wont-fix/issues.png)

A three year old bug that still wasn't fixed. The expert of `postgresql-libpq` `lpsmith` had weighed in on the bug. `cocreature` and `lpsmith` had discovered a workaround. The workaround would require inserting `threadWaitRead` between sending a query and checking for the result in the bowels of `hasql`. They doubted `nikita-volkov` would merge it though.

I started by seeing if the server was sending the notification or not using Wireshark.
...
To see if the client was recieving the notification I could use something like `strace` but there would be a lot of noise and I don't believe `strace` can show the actual data returned by `recvfrom`.

So I opted for `bpftrace` instead.

`bpftrace` is part of a suite of new Linux observability tools built off the `ebpf` virtual machine and Linux probe technologies.

I wrote the following `bpftrace` script:
...
> Currently the `buf()` requires the latest version on GitHub which is not yet release but building it from source is easy.

Another thing to point out `tracepoint`s arguments require specific names. I had to lookup the format of `sys_enter_recvfrom` by reading the "format" file in the appropiate `/sys/kernel/debug/tracing/events/` folder.
...
tester:w tracepoint:syscalls:sys_exit_recvfrom tid 3692 message C\x00\x00\x00\x0bNOTIFY\x00A\x00\x00\x00\x14\x00\x00\x0e~channel\x00hi!\x00Z\x00\x00\x00\x05I ret 39
...
```
...
Anyway, if we are recieving the notification so why doesn't `epoll_wait` get notified?
...
kprobe:sock_def_readable
{
  ...
  printf("%s sock_def_readable tid %d port %d\n", comm, tid, $dport);
}
...
The first argument to `socket_def_readable` is a `sock*` but we can also cast it a `inet_sock*` (for some reason I find this works better to get the destination port. Also you can get the source port this way).
...
I had to convert the port from big to little endian. That is what the bit shifting is about. At the time of writing this blog these operators were still undocumented.

Anwyay running  `bpftrace epoll_wait_debugger.bt` I get:

```
tester:w sock_def_readable tid 4505 port 2222
...
I extend the script with probes for `epoll_wait`.
...
The `EventManager` runs a abound thread with a specific name (TODO link to the code) so I filter `epoll_wait` calls using the name of the `EventManager` thread.
...
The triple calls to `epoll_wait` are a small optimization (hopefully) of the `EventManager` implementation to conserve OS threads. It's not important. The important piece is that we call `epoll_wait` but it never returns.

Okay so time to very some assumptions. Was the file descriptor ready event actually registered? The registeration requires a call to `epoll_ctl`. Time for more probes:
...
This is the expected sequence. The `epoll_ctl` successfully added a callback after modifying an existed one failed because it didn't exist.

There goes that theory.

Not sure what was going I started to read the `epoll_wait` source. Not sure which code paths were executed I started to add more kprobes to see what `epoll_wait` was doing.

This when I noticed the following sequence:

```
tester:w sock_def_readable tid 5902 port 2222
tester:w sock_def_readable exit tid 5902
tester:w sys_enter_recvfrom tid 5902 fd 13
tester:w sys_exit_recvfrom tid 5902 message C\x00\x00\x00\x0bNOTIFY\x00A\x00\x00\x00\x14\x00\x00\x17 channel\x00hi!\x00Z\x00\x00\x00\x05I ret 39
kprobe:ep_send_events_proc tid 5900
tester:w kprobe:ep_item_poll.isra.0 tid 5900
tester:w kretprobe:ep_item_poll.isra.0 tid 5900
kretprobe:ep_send_events_proc tid 5900
```
...
Looking at the code (TODO) add a link it seemed like the `epoll_wait` thread woke up and check to see what the events were and then went back to sleep.

What can clear out events. Closing a file descriptor could.

I traced out `closes`. Nothing out of the ordinary.
...
Reading through the code for `recvfrom` I could not see how it was doing this. Then I happened to reread this blog post and it talked about a similar situation with write ready events and mentioned the function `tcp_poll`.

`tcp_poll` was the missing piece of the puzzle. I added the probe and saw that when `ep_item_poll` called and before `recvfrom` ran `tcp_poll` returned `325` which is equivalent the event flags:
...
This is cleaned version of the steps I took. It's not really in order. In reality, I started by ignoring the problem and convincing myself it couldn't affect me. Then I tried to use DTrace and failed. DTrace never seems to work well on Macs. So I moved to Linux and recompiled GHC and added tracing to the `EventManager` and dumped the eventlog.

I decided this was an oppurtunity to read "BPF Performance Tools" and it was helpful. I starting to dive through the Linux network stack using the `bpftrace`'s `kstack` call to understand the sequences of functions for sending and recieving data on localhost. I learned a lot. It was very interesting. It was an inefficent way to solve this bug.

I made a bunch of wrong turns but steps above are what ulimately helped after I tried many things that didn't.

# Finally words on postgres notifications.

Back the bug that started this all.

Now that I understand the mechanics of the issue can the bug be fixed?

There are two ways to avoid the bug that I see.

One is two use a simpler implementation of waiting for a PostgreSQL notification, which assumes the connection used for waiting will not be used for anything else. This is the approach I took with `libpq-postgresql-notify`.

`lpsmith`'s original notification design is very clever. It is written in a way where it, in theory, it can wait for a notification while freeing the connection up for other operations.

However due to the semantics of `threadWaitRead`, we have to be very careful no calls to `recvfrom` are made without `epoll_wait` returning first. It might be possible to do this using the approach `cocreature` developed where a `threadWaitRead` is placed between calls to `sendQuery` and `getResult`. However care will have to be taken to do the same with `consumeInput` or anythiing else that can call `recvfrom` on the connection anywhere inside the `libpq` codebase. I don't have enough `libpq` mastery to know for sure this approach will succeed but I'm hopeful it.
